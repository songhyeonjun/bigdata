...
setwd('../03.Output/') #- set work directory

#### Import Data ####

#### yc.csv ####
yc_df <- read_csv('yc.csv') 
yc_df$yc_bin <- ifelse(yc_df$yc >0 , 1, 0)
yh_cut <- c(max(yc_df$yh)-2, #train Priod  1
            max(yc_df$yh)-1, #train Priod  2
            max(yc_df$yh) #predict Preiod 
            )
...
#### yh*_cn.csv ####
yh_cn_check <- dir(pattern = '_cn.csv') %>% sort()
...
#### yh*_mov.csv ####
yh_mov_check <- dir(pattern = '_mov.csv') %>% sort()

mov_ls <- lapply(yh_mov_check, read_csv) 

for(i in 1:length(mov_ls)) {
  mov_ls[[i]] <- left_join(doro_info %>% select(split_id),
                           mov_ls[[i]]) %>% na_df()
  mov_ls[[i]]$yh <- yh_cut[i]
  
}

cn_df <- full_join(bind_rows(cn_ls),
                   bind_rows(mov_ls) ) %>% na_df()



#### yh*_work.csv ####
# work_file_yn <- dir(pattern = '_work.csv') %>% sort()
# if (length(work_file_yn) > 0) {
#   work_df <-  lapply(work_file_yn, read_csv) 
#   
#   for (j in length(work_df)){
#     work_df[[j]]$yh <- yh_cut[j] 
#   }
#  
...
cn_df <- cn_tmp %>% select(-RDS_MAN_NO) %>%
  mutate(
...
result_table_final <- result_table %>% select(yc, RDS_MAN_NO, split_id, park_lot, m500_cn_n, xg.pred_h1, xg.pred_h2) %>% rename(m500_park_lot=park_lot, m500_cn=m500_cn_n)

# work.csv 파일이 없을 경우 넘어감
if(result_table$m500_cn_work) {
  result_table_final <- result_table_final %>% mutate(m500_cn_work=result_table$m500_cn_work) %>% select(yc, RDA_MAN_NO, split_id, m500_park_lot, m500_cn_n, m500_cn_work, xg.pred_h1, xg.pred_h2)
}
...
write.csv(result_table_final,
          paste0('./', folder_nm, '/result_table_final.csv'),
          row.names = F) 
